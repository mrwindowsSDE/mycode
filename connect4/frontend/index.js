(function () {
  var ConnectFour = function () {
    ...
    var markNextFree = function (x) {
      ...
      fetch(`http://localhost:2226/placepiece/${nextY}/${x}`, {
        method: "get",
        mode: "cors",
      })
        ...
        .then((data) => {
          ...
          if (data.ok) {
            ...
            document
              .querySelector("#column-" + x + " .row-" + nextY + " circle")
              .setAttribute("class", currentPlayer);
            if (data.won) {
    ...
    var isBounds = function (x, y) {
      return (
        gameBoard.hasOwnProperty(x) && typeof gameBoard[x][y] !== "undefined"
      );
    };
    ...
    var checkDirection = function (currentX, currentY, direction) {
      ...
      directions[direction].forEach(function (coords) {
        ...
        while (
          isBounds(currentX + coords[0] * i, currentY + coords[1] * i) &&
          gameBoard[currentX + coords[0] * i][currentY + coords[1] * i] ===
            currentPlayer
        ) {
